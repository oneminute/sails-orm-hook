...
module.exports = function validateDatastoreConfig(
  datastoreIdentity,
  hook,
  sails
) {
  ...
  if (_.isObject(datastoreConfig.adapter)) {
    ...
    if (!datastoreConfig.adapter.identity) {
      throw new Error(
        "Consistency violation: the adapter for datastore `" +
          datastoreIdentity +
    ...
    if (
      hook.adapters[datastoreConfig.adapter.identity] &&
      hook.adapters[datastoreConfig.adapter.identity] !==
        datastoreConfig.adapter
    ) {
      throw new Error(
        "Consistency violation: attempted to load two different data adapters with the identity `" +
          datastoreConfig.adapter.identity +
          "`."
      );
    }
    ...
    datastoreConfig.adapter = validateAdapter(
      datastoreConfig.adapter,
      adapterIdentity,
      datastoreIdentity
    );
  ...
  if (hook.adapters[adapterIdentity]) {
    _.extend(
      ...
      hook.adapters[adapterIdentity].defaults
    );
  }
