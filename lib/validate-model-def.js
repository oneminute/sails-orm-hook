...
var VALIDATIONS = require('waternile/accessible/allowed-validations');
...
module.exports = function validateModelDef(
  originalModelDef,
  modelIdentity,
  hook,
  sails
  ...
  if (!modelIdentity.match(/^[a-z_][a-z0-9_]*$/)) {
    throw flaverr(
      { name: 'userError', code: 'E_INVALID_MODEL_DEF' },
      new Error(
        '\n-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n' +
          'The `' +
          originalModelDef.globalId +
          '` model has an invalid name.\n' +
  ...
  if (
    !_.isFunction(sails.config.models.attributes) &&
    !_.isArray(sails.config.models.attributes) &&
    _.isObject(sails.config.models.attributes)
  ) {
    normalizedModelDef.attributes = _.extend(
      normalizedModelDef.attributes,
  ...
  if (
    !_.isFunction(originalModelDef.attributes) &&
    !_.isArray(originalModelDef.attributes) &&
    _.isObject(originalModelDef.attributes)
  ) {
    normalizedModelDef.attributes = _.extend(
      normalizedModelDef.attributes,
      originalModelDef.attributes
    );
  }
  ...
  if (
    !_.isUndefined(normalizedModelDef.autoCreatedAt) ||
    !_.isUndefined(normalizedModelDef.autoUpdatedAt) ||
    !_.isUndefined(normalizedModelDef.autoPK)
  ) {
    throw flaverr(
      { name: 'userError', code: 'E_INVALID_MODEL_DEF' },
      new Error(
    ...
    throw flaverr(
      ...
      new Error(
        '\n-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n' +
          ...
          normalizedModelDef.primaryKey +
          '`, but no such attribute was found on the model.\n' +
          'You must define an `' +
          normalizedModelDef.primaryKey +
          '` attribute in `api/' +
          originalModelDef.globalId +
          '.js` or in `config/models.js`.\n' +
  ...
  _.each(normalizedModelDef.attributes, function updateProperties(
    val,
    attributeName
  ) {
    ...
    if (val.collection || val.model) {
      if (!_.isUndefined(val.validations)) {
        ...
        sails.log.debug(
          'The `validations` property is not valid for associations.  Ignoring...\n'
        );
    ...
    _.each(val, function handleTopLevelValidations(property, propertyName) {
      ...
      if (VALIDATIONS[propertyName]) {
        ...
        if (val.collection || val.model) {
          ...
          sails.log.debug(
            'The `' +
              propertyName +
              '` property is not valid for associations.  Ignoring...\n'
          );
      ...
      if (DEPRECATED_VALIDATIONS[propertyName]) {
        ...
        sails.log.debug(
          '  The `' +
            propertyName +
            '` validation rule is now `' +
            DEPRECATED_VALIDATIONS[propertyName] +
            '` (changing it for you this time).\n'
        );
        val.validations = val.validations || {};
    ...
    _.each(_.keys(val.validations), function(validation) {
        ...
        throw flaverr(
          ...
          new Error(
            '\n-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n' +
              ...
              'The `' +
              validation +
              '` validation rule is not a supported validation rule.\n' +
              'Supported validation rules are: \n' +
              _.keys(VALIDATIONS).reduce(function(a, v) {
                a += ' - ' + v + '\n';
                return a;
              }, '') +
              '-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n'
        ...
        throw flaverr(
          ...
          new Error(
            '\n-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n' +
              ...
              'The `' +
              validation +
              '` validation rule does not apply to the `' +
              val.type +
              '` attribute type.\n' +
              '-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n'
          )
      ...
      if (!_.isFunction(rule.checkConfig)) {
        throw new Error(
          'Consistency violation: Rule is missing `checkConfig` function!  (Could an out-of-date dependency still be installed?  (To resolve, try running `rm -rf node_modules && rm package-lock.json && npm install`.)'
        );
      }
        ...
        throw flaverr(
          ...
          new Error(
            '\n-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n' +
              ...
              modelIdentity +
              '`:\n' +
              'The configuration provided for the `' +
              validation +
      ...
      throw flaverr(
        ...
        new Error(
          '\n-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n' +
            ...
            'The type "' +
            val.type +
            '" is no longer supported.  To use this type in your model, change\n' +
            '`type` to one of the supported types and set the `columnType` property to a column \n' +
            'type supported by the model\'s adapter, e.g. { type: \'' +
            noLongerSupportedTypes[val.type].suggestType +
            '\', columnType: \'' +
            noLongerSupportedTypes[val.type].suggestColumnType +
            '\' }\n' +
            '-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n'
        )
      );
    ...
    if (typeMapping[val.type]) {
      sails.log.debug(
        'In model `' +
          ...
          '`, the `' +
          attributeName +
          '` attribute declares deprecated type `' +
          val.type +
          '`.'
      );
      sails.log.debug(
        'Please update to a known type (changing to `' +
          typeMapping[val.type] +
          '` for now). If you wish to use'
      );
      sails.log.debug(
        'a specific column type supported by your database, set the `columnType` property'
      );
      sails.log.debug(
        '(otherwise the adapter will choose an appropriate column type automatically, if relevant).\n'
      );
    ...
    val.autoMigrations.unique =
      val.autoMigrations.unique ||
      normalizedModelDef.primaryKey === attributeName ||
      false;

    // Set the `autoIncrement` autoMigration property to `false` if it's not already configured.
    val.autoMigrations.autoIncrement =
      val.autoMigrations.autoIncrement || false;
  ...
  if (
    ...
    normalizedModelDef.migrate = 'safe';
    sails.log.verbose(
      'For `%s` model, forcing Waterline to use `migrate: "safe" strategy (since this is production)',
      modelIdentity
    );
  }
  ...
  if (!_.isString(normalizedModelDef.datastore)) {
    throw new Error(
      '\n-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n' +
        ...
        'Instead, got: ' +
        util.inspect(normalizedModelDef.datastore, { depth: null }) +
        '\n' +
        (_.isArray(normalizedModelDef.datastore)
          ? '(Models can only use one datastore at a time.)\n'
          : '') +
        '-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n'
    );
  }
  ...
  var normalizedDatastoreConfig =
    hook.normalizedDSConfigs[normalizedModelDef.datastore];
  if (!_.isObject(normalizedDatastoreConfig)) {
    throw new Error(
      'A model (`' +
        modelIdentity +
        '`) references a datastore which cannot be found (`' +
        normalizedModelDef.datastore +
        '`).  If this model definition has an explicit `datastore` property, check that it is spelled correctly.  If not, check your default `datastore` (usually located in `config/models.js`).  Finally, check that this datastore (`' +
        normalizedModelDef.datastore +
        '`) is valid as per http://sailsjs.com/config/datastores.'
    );
  }
  ...
  if (normalizedDatastoreConfig.adapter === 'sails-mongo') {
    ...
    if (
      normalizedModelDef.datastore !== 'default' ||
      (originalModelDef.attributes &&
        originalModelDef.attributes[normalizedModelDef.primaryKey])
    ) {
      var primaryKeyAttr =
        normalizedModelDef.attributes[normalizedModelDef.primaryKey];
      if (
        primaryKeyAttr.autoIncrement ||
        (primaryKeyAttr.type !== 'string' &&
          normalizedModelDef.dontUseObjectIds !== true) ||
        primaryKeyAttr.columnName !== '_id'
      ) {
        sails.log.debug('In model `' + modelIdentity + '`:');
        sails.log.debug(
          'The default primary key attribute (`' +
            normalizedModelDef.primaryKey +
        ...
        sails.log.debug(
          'Also, in most cases (unless `dontUseObjectIds` has been set to `true` for the model),'
        );
        sails.log.debug(
          'then the `type` of the primary key must also be `string`.'
        );
        sails.log.debug();
  ...
  normalizedModelDef.getDatastore = function() {
    if (arguments.length > 0) {
      throw new Error(
        'The `getDatastore()` model method should be called with no arguments.  (To look up a particular datastore by name, use `sails.getDatastore()`.)'
      );
    }
  ...
  normalizedModelDef.native = function(done) {
    ...
    if (!done) {
      ...
      throw new Error(
        'Invalid callback function provided when invoking .native().'
      );
    }
    ...
    var isUsingSailsMongo = (function __get__() {
      var normalizedDatastoreConfig =
        hook.normalizedDSConfigs[normalizedModelDef.datastore];
      var adapterIdentity =
        hook.adapters[normalizedDatastoreConfig.adapter].identity;
    ...
    if (!isUsingSailsMongo) {
      return done(
        new Error(
          'This model (`' +
            normalizedModelDef.identity +
            '`) does not appear to be using the `sails-mongo` adapter, so the `.native()` method is not available.  (See http://sailsjs.com/docs/reference/waterline-orm/models/native)'
        )
      );
    }
    ...
    // Sanity check:
    if (
      !normalizedModelDef.tableName ||
      !_.isString(normalizedModelDef.tableName)
    ) {
      return done(
        new Error(
          'Consistency violation: Models should always have a valid `tableName` at this point, but this model (`' +
            normalizedModelDef.identity +
            '`) does not...'
        )
      );
  ...
  normalizedModelDef.query = function(sql, valuesToEscape, done) {
    ...
    if (!done) {
      ...
      throw new Error(
        'Invalid callback function provided when invoking .query().'
      );
    }
    ...
    normalizedModelDef
      .getDatastore()
      .sendNativeQuery(sql, valuesToEscape, done);
  };
